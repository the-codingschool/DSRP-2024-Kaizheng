...
```
```{r}
accident_info <- select(traffic, c(day_of_week, time_cat, primary_rd, secondary_rd, distance, collision_severity, type_of_collision, ped_action, road_surface, lighting, number_killed, number_injured, party_type, party_sex, party_age, party_sobriety, race, at_fault))

accident_info


CHIS <- lapply(accident_info[,-18], function(x) chisq.test(table(accident_info[,18], x)))
