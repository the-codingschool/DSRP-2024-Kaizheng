...
```{r}
MLdata <- traffic_data |>
  select("time_cat","number_injured", "party_age", "collision_severity") |>
  filter(collision_severity != "Medical") |>
  na.omit()
MLdata <-
  MLdata |>
...
```{r}
lr_model <- lm(new_col_sever ~ time_cat + number_injured + party_age, data = train_data)
#predict
...
test_data_filtered <- test_data %>%
  filter(!(new_col_sever == 1 & pred > 1.4))
ggplot(test_data_filtered, aes(x = new_col_sever, y = pred)) +
  geom_point() +
  geom_abline(linetype = "dashed", color = "red") +
  labs(title = "Real vs Predicted Values Using time, age, and number injured",
